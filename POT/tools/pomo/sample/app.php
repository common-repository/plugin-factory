...
function __($text, $domain = 'default', 'plugin-factory') {
	$translations = &get_translations($domain);
	return $translations->translate($text);
}

function _e($text, $domain = 'default', 'plugin-factory') {
	$translations = &get_translations($domain);
...
printf(__('Welcome %s!', 'plugin-factory')."\n", $user);

printf(__n('You have one new message.', 'You have %s new messages.', $messages)."\n", $messages);

echo __("A string with low priority!", 'side', 'plugin-factory')."\n";
